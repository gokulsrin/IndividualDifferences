...
function reformatData(data) {
    ...
    while(i <= num_warmup_trials) {
        var trialData = {};
        //demographic info
        trialData.subject_id = demo1["id"]
        trialData.age = demo1["age"];
        trialData.gend = demo1["gend"];
        trialData.hand = demo1["hand"];
        trialData.education = demo1["education"];
        //trial specific info
        trial = data[i];
    ...
    for(var j=0; j<num_vignettes; j++) {
        ...
        view_time = trial["rt"];
        i=i+1;
        for(var k=0; k<num_q_per_vignette; k=k+2) {
            trial = data[i];
            //demographic info
            trialData.subject_id = demo1["id"]
            trialData.age = demo1["age"];
